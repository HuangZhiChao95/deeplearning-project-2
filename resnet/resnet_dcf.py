...
def conv3x3(in_planes, out_planes, stride=1):
    ...
    -- we replace it by Conv_DCF
    """
    return Conv_DCF(in_planes, out_planes, kernel_size=3, padding=1, bias=False, stride=stride)
...
def resnet101(pretrained=False, in_channels=1, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 4, 23, 3], in_channels, **kwargs)
    if pretrained:
...
def resnet152(pretrained=False, in_channels=1, **kwargs):
    ...
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 8, 36, 3], in_channels, **kwargs)
    if pretrained:
